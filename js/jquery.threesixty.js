    ...
    var scope,
        ...
        $.fn.nextFrame = ThreeSixty.prototype.nextFrame = function(){
            $(this).each(function(i){
                ...
                $this.find('.threesixty-frame').css({opacity: 0});
                $this.find('.threesixty-frame:eq(' + val + ')').css({opacity: 1});
            });
        };
        ...
        ThreeSixty.prototype.onLoadAllComplete = function(objIndex) {
            ...
            // add 360 images
            // for(i; i < l; i++){
            for(i = l - 1; i >= 0; i--){ // ZOU  相反转动对应
                var opacity = (i === 0) ? '1' : '0';
                html += '<img class="threesixty-frame" style="opacity:' + opacity + ';" data-index="' + i + '" src="' + pathTemplate.replace('{index}', i) + '"/>';
            }
            $this.html(html);
        ...
        ThreeSixty.prototype.onMove = function(screenX, screenY){
            if(isMouseDown){
                ...
                $downElem.find('.threesixty-frame').css({opacity: 0});
                $downElem.find('.threesixty-frame:eq(' + val + ')').css({opacity: 1});
            }
        };
